...
export default function DynamicGrid({ columns = [], apiUrl, Module }) {
      ...
      .then((response) => {
        if (response.status === 200) {
          ...
          const list = Array.isArray(dataObj)
            ...
            : [];
          setData(list);
          setTotalPages(typeof dataObj === "object" && dataObj?.totalPages ? dataObj.totalPages : 1);
  ...
  return (
    <div className="grid-wrapper">
      ...
      <div className="grid-container">
        <table className="grid-table">
          ...
          <thead>
            <tr>
              {columns.map((col, idx) => (
                <th key={idx} title={col.header}>{col.header}</th>
              ))}
            </tr>
          </thead>
        </table>
          ...
          <table className="grid-table">
            ...
            <tbody>
              {emptyMsg && filteredData.length === 0 ? (
                <tr>
                  <td colSpan={columns.length} style={{ textAlign: "center", padding: "20px", color: "#666", fontSize: "15px" }}>
                    {emptyMsg}
                  </td>
                </tr>
              ) : (
                filteredData?.map((row, rowIndex) => (
                  <tr key={rowIndex}>
                    {columns.map((col, colIndex) => {
                        ...
                        return (
                          <td key={colIndex} className="action-column">
                            {selectedStatus === "inactive" ? (
                              <>
                                <span className="action-icon" title="View" onClick={() => handleView(row)} style={{ cursor: "pointer" }}>
                                  <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#504f4fff">
                                ...
                                <span className="action-icon" title="Activate" onClick={() => handleActivate(row)} style={{ cursor: "pointer" }}
                                  dangerouslySetInnerHTML={{ __html: '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#2da109ff"><path d="M382-240 154-468l57-57 171 171 367-367 57 57-424 424Z"/></svg>' }} />
                              </>
                            ) : (
                              <>
                                <span className="action-icon" title="Edit" onClick={() => handleEdit(row)} style={{ cursor: "pointer" }}>
                                  <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#504f4fff">
                                ...
                                {Module === "Sales" ? (
                                  <span className="action-icon" title="Download" onClick={() => handleDownload(row)} style={{ cursor: "pointer" }}
                                    dangerouslySetInnerHTML={{ __html: '<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#504f4fff"><path d="M480-320 280-520l56-58 104 104v-326h80v326l104-104 56 58-200 200ZM240-160q-33 0-56.5-23.5T160-240v-120h80v120h480v-120h80v120q0 33-23.5 56.5T720-160H240Z"/></svg>' }} />
                                ) : (
                                  <span className="action-icon" title="Disable" onClick={() => handleDisable(row)} style={{ cursor: "pointer" }}>
                      ...
                      return (
                        <td key={colIndex} title={value !== undefined && value !== null ? typeof value === "object" ? JSON.stringify(value) : String(value) : ""}>
      ...
      {isViewOpen && <ViewModal isOpen={isViewOpen} onClose={() => { setIsViewOpen(false); setViewUkey(null); }} moduleName={Module} uKey={viewUkey} />}
