  ...
  return (
    <div className="pfx-drop-wrap" ref={dropRef}>
      <div className={`pfx-drop-box ${open ? "pfx-drop-open" : ""}`} onClick={() => { if (!open) setSearch(""); setOpen(!open); }}>
        <input className="pfx-drop-input"
      ...
      {open && (
        <ul className="pfx-drop-list">
          {filtered.map(o => (
            <li key={o.id} className={o.id === value ? "pfx-drop-selected" : ""}
...
export default function PurchaseEdit({ uKey, onSubmit, onClose }) {
  ...
  const [form, setForm] = useState({
    supplierId: "", supplierInvoiceNumber: "", invoiceDate: "",
    paymentStatus: "CREDIT", dueDate: "", amountPaid: "", notes: "", items: [],
  });

  /* ── FETCH DROPDOWNS ── */
  useEffect(() => {
    fetch("http://localhost:8080/api/suppliers/dropdown", { credentials: "include" })
  ...
  useEffect(() => {
    if (!uKey) return;
    setFetching(true);
    fetch(`http://localhost:8080/api/Purchase/GetPurchaseByUKey/${uKey}`, { credentials: "include" })
      .then(r => r.json())
      .then(json => {
        if (json?.status === 200 && json.data) {
          ...
          setForm({
            ...
            paymentStatus:         d.paymentStatus         || "CREDIT",
            dueDate:               d.dueDate               || "",
  ...
  const addItem = () => setForm(p => ({
    ...p,
    items: [...p.items, { productId: "", productName: "", quantity: 1, costPrice: "", mrp: "", gstRate: 0, batchNumber: "", manufacturingDate: "", expiryDate: "" }]
  }));
  ...
  const getLineTotal = item => {
    ...
    return base + base * (parseFloat(item.gstRate) || 0) / 100;
  };
  ...
  const validate = () => {
    ...
    form.items.forEach((item, i) => {
      ...
      if (!item.quantity)    ie.quantity = "Required";
      if (!item.costPrice)   ie.costPrice = "Required";
      if (!item.mrp)         ie.mrp = "Required";
      if (!item.batchNumber) ie.batchNumber = "Required";
  ...
  const handleSubmit = async () => {
    if (!validate()) { toast.error("Please fix errors before submitting"); return; }
    setLoading(true);
    try {
      const payload = {
      ...
      const res  = await fetch(`http://localhost:8080/api/Purchase/UpdatePurchase/${uKey}`, {
      ...
      <div className="pfx-modal">
              ...
              {activeTab === "DETAILS" && (
                <div className="pfx-details-grid">
                  <div className="pfx-field">
                    ...
                    <SearchDrop options={suppliers} value={form.supplierId} onChange={v => setField("supplierId", v)}
                      placeholder="Search supplier…" dropRef={supplierRef} open={supplierOpen} setOpen={setSupplierOpen} />
                  ...
                  <div className="pfx-field">
                    ...
                    <input className={`pfx-input ${errors.supplierInvoiceNumber ? "pfx-input-err" : ""}`}
                      value={form.supplierInvoiceNumber} onChange={e => setField("supplierInvoiceNumber", e.target.value)} placeholder="e.g. INV-2024-001" />
                  ...
                  {form.paymentStatus === "PARTIAL" && (
                    <div className="pfx-field">
                      ...
                      <input type="number" className="pfx-input" min="0"
                        value={form.amountPaid} onChange={e => setField("amountPaid", e.target.value)} placeholder="0.00" />
                    </div>
                  )}
                  ...
                  {form.items.length === 0 ? (
                    ...
                    <>
                      <div className="pfx-items-head">
                        <span>Product</span><span>Batch No</span><span>MFG Date</span>
                        <span>Expiry Date</span><span>Qty</span><span>Cost (₹)</span>
                        <span>MRP (₹)</span><span>GST %</span><span>Total (₹)</span><span></span>
                      </div>

                      {form.items.map((item, i) => (
                        <div key={i} className="pfx-item-row">
                          <div ref={el => productRefs.current[i] = el}>
                            <SearchDrop options={products} value={item.productId} onChange={v => updateItem(i, "productId", v)}
                              placeholder="Select product" dropRef={{ current: productRefs.current[i] }}
                              open={productOpenIdx === i} setOpen={v => setProductOpenIdx(v ? i : null)} />
                            {errors.items?.[i]?.productId && <span className="pfx-error">{errors.items[i].productId}</span>}
                          </div>

                          <div>
                            <input className={`pfx-cell-input ${errors.items?.[i]?.batchNumber ? "pfx-input-err" : ""}`}
                              value={item.batchNumber} onChange={e => updateItem(i, "batchNumber", e.target.value)} placeholder="Batch No" />
                            {errors.items?.[i]?.batchNumber && <span className="pfx-error">{errors.items[i].batchNumber}</span>}
                          </div>

                          <div><input type="date" className="pfx-cell-input" value={item.manufacturingDate} onChange={e => updateItem(i, "manufacturingDate", e.target.value)} /></div>

                          <div>
                            <input type="date" className={`pfx-cell-input ${errors.items?.[i]?.expiryDate ? "pfx-input-err" : ""}`}
                              value={item.expiryDate} onChange={e => updateItem(i, "expiryDate", e.target.value)} />
                          ...
                          <div>
                            <input type="number" min="0" className={`pfx-cell-input ${errors.items?.[i]?.costPrice ? "pfx-input-err" : ""}`}
                              value={item.costPrice} onChange={e => updateItem(i, "costPrice", e.target.value)} placeholder="0.00" />
                          ...
                          <div>
                            <input type="number" min="0" className={`pfx-cell-input ${errors.items?.[i]?.mrp ? "pfx-input-err" : ""}`}
                              value={item.mrp} onChange={e => updateItem(i, "mrp", e.target.value)} placeholder="0.00" />
                            {errors.items?.[i]?.mrp && <span className="pfx-error">{errors.items[i].mrp}</span>}
                          </div>

                          <div>
                            <select className="pfx-cell-input" value={item.gstRate} onChange={e => updateItem(i, "gstRate", e.target.value)}>
                              {GST_RATES.map(r => <option key={r} value={r}>{r}%</option>)}
                            </select>
                          </div>

                          <div className="pfx-line-total">₹{getLineTotal(item).toFixed(2)}</div>

                          <button type="button" className="pfx-remove-btn" onClick={() => removeItem(i)}>
                      ...
                      <div className="pfx-totals">
                        <div className="pfx-totals-row"><span>Subtotal</span><span>₹{(grandTotal - totalTax).toFixed(2)}</span></div>
                        <div className="pfx-totals-row"><span>Total GST</span><span>₹{totalTax.toFixed(2)}</span></div>
                        <div className="pfx-totals-divider" />
                        <div className="pfx-totals-row pfx-totals-grand"><span>Grand Total</span><span>₹{grandTotal.toFixed(2)}</span></div>
                      </div>
                    </>
                  )}
        ...
        <div className="pfx-footer">
          ...
          <button type="button" className="pfx-btn-primary" onClick={handleSubmit} disabled={loading || fetching}>
            {loading ? <><span className="pfx-spinner" /> Updating…</> : <>
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
