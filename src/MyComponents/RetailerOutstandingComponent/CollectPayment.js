...
function InvoiceRow({ invoice, active, onCollect, onCancel, onSuccess }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (json.status === 200) {
        toast.success(`Rs ${fmt(parsed)} collected via ${mode}`);
        onSuccess();
      } else {
    ...
    <div className={`cp-ledger-invoice ${active ? "cp-ledger-invoice-active" : ""}`}>
      <div className="cp-ledger-invoice-main">
        ...
        <div className="cp-ledger-actions">
          ...
          {remaining > 0 && !active && (
            <button className="cp-btn-primary cp-btn-compact" onClick={() => onCollect(invoice.salesId)}>
              Collect Payment
            </button>
          )}
        </div>
        ...
        <div className="cp-collect-panel">
          ...
          <div className="cp-panel-actions">
            ...
            <button className="cp-btn-primary" onClick={submit} disabled={submitting}>
              {submitting ? <><span className="cp-spinner" /> Collecting</> : `Collect Rs ${fmt(amount || 0)}`}
            </button>
          </div>
        </div>
      )}
...
export default function CollectPayment({ uKey, onClose, onSuccess }) {
          ...
          {!loading && !error && ledger && (
            <>
              <div className="cp-status-row">
                <span className="cp-code-badge">{ledger.retailerCode}</span>
              ...
              <div className="cp-tabs">
                {[
                  ...
                  <button
                    ...
                    onClick={() => { setActiveTab(tab.key); setCollectingId(null); }}
                  >
                    {tab.label}
                    <span>{tab.count}</span>
                  </button>
              ...
              {activeTab === "unpaid" && (
                <div className="cp-ledger-list">
                  {unpaidInvoices.length === 0 ? (
                    <div className="cp-ledger-empty">No unpaid invoices. This retailer is settled.</div>
                  ) : (
                    unpaidInvoices.map(inv => (
                      <InvoiceRow
                        key={inv.salesId || inv.invoiceNumber}
                        invoice={inv}
                        active={collectingId === inv.salesId}
                        onCollect={setCollectingId}
